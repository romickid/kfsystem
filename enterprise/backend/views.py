...
def customer_get_web_url(request):
    if request.method == 'POST':
        ...
        data = 'http://192.168.55.33:8000/web/demo/?userID=' + email + '&nickname=' + nickname + '&telephone=' + telephone + '&location=' + location + '&description=' + description + '&signature=' + signature
...
def customer_get_widget_url(request):
    if request.method == 'POST':
        ...
        hash_key.update(('demo'+email+nickname+communication_key).encode('utf-8'))
        signature = hash_key.hexdigest()
        data = 'http://192.168.55.33:8000/widget/demo/?userID=' + email + '&nickname='+ nickname + '&telephone=' + telephone + '&location=' + location + '&description=' + description + '&signature=' + signature
        return HttpResponse(data, status=200)
...
def customer_get_mobile_url(request):
    if request.method == 'POST':
        ...
        hash_key.update(('demo'+email+nickname+communication_key).encode('utf-8'))
        signature = hash_key.hexdigest()
        data = 'http://192.168.55.33:8000/mobile/demo/?userID=' + email + '&nickname=' + nickname + '&telephone=' + telephone + '&location=' + location + '&description=' + description + '&signature=' + signature
        return HttpResponse(data, status=200)
