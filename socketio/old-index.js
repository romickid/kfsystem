...
io.on('connection', function (socket) {
  ...
  */
  socket.on('server message', function (msg, isText, img, bigImg, fromId, toId) {
    ...
    to[0].emit('server message', msg, isText, img, bigImg, fromId, toId)
    console.log('Server ' +  fromId + ' send message to customer ' + toId)
  })

  /**
  * @description 用户向客服发送信息
  */
  socket.on('customer message', function (msg, isText, img, bigImg, fromId, toId) {
    let to = findById(serverSocket, toId)
    to[0].emit('customer message', msg, isText, img, bigImg, fromId, toId)
  ...
  */
  ...
  /**
  * @description 用户关闭页面：从客服端发出断开客户的信息
  */
